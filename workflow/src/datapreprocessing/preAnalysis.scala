...
class preAnalysis extends Serializable{
  ...
  def add(sample:Vector): this.type = {
    ...
    require(n == sample.size, s"当前加入的数据维度与之前数据维度不匹配." +
      s" 之前维度 $n 当前维度 ${sample.size}.")
  ...
  def merge(other: preAnalysis): this.type = {
    if(this.totalCnt != 0 && other.totalCnt != 0){
      require(n == other.n, s"当合并其他数据时产生了数据维度不匹配的错误 " +
        s"当前数据维度 $n 其他数据维度 ${other.n}.")
  ...
  def notNumCount : Vector = Vectors.dense(notNum.data) 
